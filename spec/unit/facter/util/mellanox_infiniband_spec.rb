...
describe Facter::Util::MellanoxInfiniband do
  ...
  describe 'lspci' do
    it 'returns output' do
      allow(Facter::Util::Resolution).to receive(:exec).with('lspci 2>/dev/null').and_return('foo')
  ...
  describe 'count_ib_devices' do
    it 'returns count' do
      ...
      expect(described_class.count_ib_devices).to eq(2)
    end

    it 'returns count for ConnectX-3' do
      allow(Facter::Util::Resolution).to receive(:which).with('lspci').and_return('/usr/bin/lspci')
      allow(described_class).to receive(:lspci).and_return(my_fixture_read('mellanox_lspci_2'))
